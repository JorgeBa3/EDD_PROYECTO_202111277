...
contains
            ...
            if(encabezadoFila%j == nodo%j) then
                ...
                do while(associated(columna)) 
                    if(columna%i == nodo%i) then
                        columna%valor%value = nodo%valor%value
                        existe = .true.
                        return
                    end if
    ...
    subroutine graficar(self)
        ...
        character(:), allocatable :: rank_filas
        character(:), allocatable :: rank_columnas
        ...
        write(io, *) 'node[shape = "box", width=0.5, height=0.5]'
        write(io, *) 'rankdir=LR'
    
        do while (associated(fila_aux))
            rank_filas = "{rank=same"
            ...
            do while(associated(columna_aux)) 
                ...
                if (columna_aux%i == -1 .and. columna_aux%j == -1) then
                    ...
                    ! Creamos un nodo especial con el color como relleno y tamaño de letra pequeño
                    contenido = 'style="filled", fillcolor="'//trim(adjustl(columna_aux%valor%value))//'", fontsize=1'
                    node_dec = trim(adjustl(nombre))//'['//trim(adjustl(contenido))//', group="'//trim(adjustl(str_i))//'"]'
                end if
                ...
                rank_columnas = rank_columnas // ';"Nodo'//trim(adjustl(str_i))//'_'//trim(adjustl(str_j))//'"'
                columna_aux => columna_aux%derecha
            end do
            rank_filas = rank_filas // rank_columnas // '}'
            write(io, *) rank_filas
        ...
        do j = 0, self%height
            ...
            do i = 0, self%width
                ...
                if(.not. val%exists) then
                    write(*, fmt='(A100)', advance='no') ""
                else
                    write(*, fmt='(A100)', advance='no') val%value
                end if
