...
contains
    ...
    subroutine crearConexion(self, origen, destino, distancia, num_impresoras)
        class(ListaAdyacencia), intent(inout) :: self
        integer, intent(in) :: origen
        integer, intent(in) :: destino
        integer, intent(in) :: distancia
        integer, intent(in) :: num_impresoras
        ...
        do while(associated(aux))
            if(aux%val == origen) then
                call aux%insertarArista(destino, distancia, num_impresoras)
                exit
            end if
            aux => aux%sig
        end do
    ...
    subroutine insertarArista(self, destino, distancia, num_impresoras)
        ...
        if(.not. associated(self%raiz)) then
            ...
            nuevo%num_impresoras = num_impresoras
        else
            aux => self%raiz
            if(destino < self%raiz%destino) then
                nuevo%sig => self%raiz
                self%raiz => nuevo
                nuevo%distancia = distancia
                nuevo%num_impresoras = num_impresoras
                ...
                do while(associated(aux%sig))
                    if(destino < aux%sig%destino) then
                        nuevo%sig => aux%sig
                        aux%sig => nuevo
                        nuevo%distancia = distancia
                        nuevo%num_impresoras = num_impresoras
